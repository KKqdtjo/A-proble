...
int main()
{
    ...
    int flag = 1;
    for (gama = 170; gama - 175 < 1e-9; gama += 0.05)
    {
        double gama_rad = gama * M_PI / 180.0;
        for (t1 = 0; t1 - 0.5 < 1e-9; t1 += 0.05)
        {
            for (t2 = 0; t2 - 1.0 < 1e-9; t2 += 0.05)
            {
                for (speed = 60; speed - 90 < 1e-9; speed += 3)
                {
                    ans = 0;
                    for (t = 0; t - 20 < 1e-9; t += 0.02)
                    {
                        flag = 1;
